...
def get_bill_from_id(bill_id: int):
    ...
    for row in cursor:
        return {
            ...
            "start_time": utils.formate_datetime_s(row[6]),
            "end_time": utils.formate_datetime_s(row[7]),
...
def get_all_bill(date: str):
    ...
    for row in cursor:
        result.append({
            ...
            "bill_date": row[2],
            "pile_id": row[3],
            "start_time": utils.formate_datetime_s(row[4]),
            "end_time": utils.formate_datetime_s(row[5]),
            "total_fee": row[6],
            "pay_state": row[7],
        })
    return result
